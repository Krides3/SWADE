...
function makeVehicleIcon(type) {
  ...
  return L.divIcon({
    html: `<div class="veh-wrap" style="transform:rotate(0deg)">${svg}</div>`,
    className: '',
    iconSize:   size,
...
class Vehicle {
  constructor(route) {
    ...
    this.marker = L.marker(initPos, { icon: makeVehicleIcon(route.type), zIndexOffset: 500 })
  ...
  tick(ts) {
    ...
    // Update rotation directly on the DOM element — no icon rebuild needed each frame
